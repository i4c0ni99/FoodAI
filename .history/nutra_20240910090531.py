...
with open('/Users/lucavisconti/Documents/FoodAI/stats/model_metrics.txt', 'w') as f:
...
for feature in features:
    ...
    plt.grid(True)
    plt.savefig('/Users/lucavisconti/Documents/FoodAI/stats/' + f'{feature}_vs_nutritional_score.png')
    plt.show()

# Genera la curva di apprendimento
train_sizes, train_scores, test_scores = learning_curve(model, X_scaled, y, cv=5, scoring='neg_mean_squared_error')

# Media dei punteggi per train e test set
train_scores_mean = -train_scores.mean(axis=1)
test_scores_mean = -test_scores.mean(axis=1)

plt.figure(figsize=(8, 6))
plt.plot(train_sizes, train_scores_mean, label='Errore di training')
plt.plot(train_sizes, test_scores_mean, label='Errore di validazione')
plt.title('Curva di Apprendimento')
plt.xlabel('Dimensione del training set')
plt.ylabel('Errore quadratico medio (MSE)')
plt.legend(loc='best')
plt.grid(True)
plt.savefig('/Users/lucavisconti/Documents/FoodAI/stats/curva_apprendimento.png')
plt.show()
...
features = ['carbohydrate', 'fat', 'protein', 'kilocalories', 'sugar_penalty', 'fiber_bonus']
plt.figure(figsize=(8, 6))
plt.barh(features, feature_importance, color='lightblue')
plt.title('Importanza delle Feature')
plt.xlabel('Coefficiente di Importanza')
plt.grid(True)
plt.savefig('/Users/lucavisconti/Documents/FoodAI/stats/importanza_feature.png')
plt.show()