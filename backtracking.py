...
def backtrack(assignment, csp):
    ...
    # Restituisce i valori dell'alimento
    dict_aliment = ret_val.to_dict(orient="records")
    assignment[var] = {  'category': dict_aliment[0]['category'], 'description' : dict_aliment[0]['description'], 
                       'carbohydrate': dict_aliment[0]['carbohydrate'],'protein':
        dict_aliment[0]['protein'],'fat' :dict_aliment[0]['fat'],'kilocalories':dict_aliment[0]['kilocalories'],
        'generic_category': dict_aliment[0]['generic_category'], 'grams' : g }
   
    return backtrack(assignment, csp)


# Cerca la tripla di alimenti
...
# Cerca l'alimento mancante nell'assignment
